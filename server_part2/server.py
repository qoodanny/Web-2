...
def handle_delete_tweet(client_socket, tweet_id):
    ...
    send_response(client_socket, '200 OK' if response['status'] == 'OK' else '500 Internal Server Error', 'Tweet deleted successfully.' if response['status'] == 'OK' else 'Tweet deletion failed.')
...
def handle_get_tweets(client_socket):
    ...
    if response['status'] == 'OK':
        print(response)
        tweets_json = json.dumps(response['tweets'])
        send_response(client_socket, '200 OK', tweets_json, content_type='application/json')
    else:
        send_response(client_socket, '500 Internal Server Error', 'Could not retrieve tweets.')


def handle_post_tweet(client_socket, body):
    data = json.loads(body)
    # Here you'd include any necessary information for the coordinator
    response = send_to_coordinator({
        'action': 'post',
        'tweet': {
            'id': str(uuid.uuid4()),
    ...
    send_response(client_socket, '200 OK' if response['status'] == 'OK' else '500 Internal Server Error', 'Tweet posted successfully.' if response['status'] == 'OK' else 'Tweet post failed.')
...
def handle_put_tweet(client_socket, tweet_id, body):
    ...
    send_response(client_socket, '200 OK' if response['status'] == 'OK' else '500 Internal Server Error', 'Tweet updated successfully.' if response['status'] == 'OK' else 'Tweet update failed.')
